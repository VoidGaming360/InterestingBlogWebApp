...
{
    public class CommentReactionRepository : RepositoryBase<CommentRecord>, ICommentReactionRepository
    {
        ...
        public async Task<CommentRecord>? GetVote(int commentId, string userId)
        {
            return await _appDbContext.CommentVotes
                    .Where(v => v.CommentId == commentId && v.UserId == userId)
                    .SingleOrDefaultAsync();
        }
        public async Task<IEnumerable<CommentRecord>> GetAllVotesForComment(int commentId)
        {
            return await _appDbContext.CommentVotes
        ...
        public async Task<CommentRecord>? GetAllVotesByUserId(string userId)
        {
            return await _appDbContext.FindAsync<CommentRecord>(userId);
        }
    }
}
